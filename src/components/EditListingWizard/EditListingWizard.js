...
export const GUIDE_TABS = [DESCRIPTION, LOCATION, ...availabilityMaybe, PHOTOS];
...
const allCustomFieldsPresent = (category, publicData) => {
  const categories = config.custom.categories;
  ...
  if (categoryBasedCustomFields && categoryBasedCustomFields.length > 0){
    categoryBasedCustomFields.forEach((field) => {
      if ( field.required_message && !publicData[field.key] )
        return false
    })
  }
  return true
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    case DESCRIPTION:
      return !!(description && title && category && allCustomFieldsPresent(category, publicData));
    case LOCATION:
...
class EditListingWizard extends Component {
  ...
  render() {
    ...
    const categoryValue = currentListing && 
      currentListing.attributes &&
      currentListing.attributes.publicData &&
      currentListing.attributes.publicData.category

    const isGuideListing = config.custom.SCHEDULING_LISTING_TYPES.includes(this.state.category || categoryValue)
    const tabs = isGuideListing ? GUIDE_TABS : TABS;
          ...
          {tabs.map(tab => {
            return (
              <EditListingWizardTab
                ...
                category={this.state.category || categoryValue}
