...
class SearchMapPriceLabel extends Component {
  ...
  render() {
    ...
    const formattedPrice =
      price && price.currency === config.currency ? formatMoney(intl, price) : price && price.currency;
    ...
    return (
      <button className={classes} onClick={() => onListingClicked(currentListing)}>
        ...
        <div className={priceLabelClasses}>{formattedPrice ? formattedPrice : title}</div>
