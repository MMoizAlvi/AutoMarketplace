    ...
    return (
      <div className={classes}>
        <div className={css.threeToTwoWrapper}>
          ...
          <RemoveImageButton onTouchStart={handleRemoveClick} onClick={handleRemoveClick} />
        </div>
      </div>
    );
  }
  ...
  return (
    <ul className={`${classes}, sortable-list`}>
      {images.map((image, index) => {
        return (
          // <SortableItem key={index} onSortItems={(e) => { onUpdateImageOrder(e.map(i => i.id.uuid))}} items={images} sortId={index}>
            <ThumbnailWrapper
              image={image}
              index={index}
