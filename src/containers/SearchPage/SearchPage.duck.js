...
export const searchListings = searchParams => (dispatch, getState, sdk) => {
  ...
  const yearValue = searchParams.pub_year && searchParams.pub_year.split(',')
  const yearMin = yearValue && parseInt(yearValue[0]);
  const yearMax = yearValue && parseInt(yearValue[1]);
  const yearMaybe = yearMin && yearMax ? {
    pub_year: [...Array(yearMax - yearMin + 1).keys()].map(i => i + yearMin).join(',')
  } : {};
