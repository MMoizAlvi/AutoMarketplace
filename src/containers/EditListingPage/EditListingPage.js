...
export const EditListingPageComponent = props => {
  ...
  if (shouldRedirect) {
    ...
    const unattachedImages = imageOrder.map(i => page.images[i]).filter(item => item);

    const allImages = currentListingImages.concat(unattachedImages).sort((a, b) => imageOrder.indexOf(a.id.uuid) > imageOrder.indexOf(b.id.uuid) ? 1 : -1);
